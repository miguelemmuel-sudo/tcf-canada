...
import { 
  BookOpen, CheckCircle2, ChevronLeft, ChevronRight, ArrowRight, BrainCircuit, Clock, Award, Sparkles, Check, X 
} from "lucide-react";
...
export default function ReadingCoursePage() {
  ...
  const LESSONS = React.useMemo<typeof BASE_LESSONS>(() => generateLessonsForPack(BASE_LESSONS, pack, PACK_CONFIGS[pack], "reading", currentLesson), [pack, currentLesson]);
    ...
    <div className="space-y-6 pb-12 max-w-4xl mx-auto px-2 sm:px-4">
      ...
      <div className="flex items-center justify-between gap-2 bg-slate-100 dark:bg-slate-900/60 p-2 rounded-xl border border-slate-200/80 dark:border-slate-800 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto py-1 flex-1 justify-center">
          {(() => {
            ...
            return getPages().map((i, idx) => {
              ...
              if (!l) return null;
              return (
                <button key={l.id || i} onClick={() => { setCurrentLesson(i); setAnswers({}); setShowResults(false); }}
                  className={`px-3.5 py-1.5 rounded-lg text-sm font-bold whitespace-nowrap transition-all flex items-center gap-1.5 ${
                    i === currentLesson
                      ...
                      : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 hover:border-emerald-400 hover:text-emerald-600 dark:text-slate-300 dark:hover:text-emerald-400"
                  }`}
                >
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-bold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-emerald-50 hover:border-emerald-300 dark:hover:bg-emerald-950/30 transition-all shadow-sm shrink-0"
