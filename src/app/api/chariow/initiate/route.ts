...
import { initiateChariowPayment } from "@/lib/chariow";
import { PACK_PRICES, PACK_NAMES } from "@/lib/fapshi";
import { createClient as createSupabaseJsClient } from "@supabase/supabase-js";

/**
 * Route d'initialisation de paiement Chariow
...
export async function POST(request: Request) {
  try {
    ...
    // 2. Initialisation auprès de Chariow
    const chariowRes = await initiateChariowPayment({
      amount: amount,
    ...
    if (adminDb && chariowRes.transId) {
      await adminDb
        .from("transactions")
        .update({ provider_transaction_id: chariowRes.transId, updated_at: new Date().toISOString() })
        .eq("reference", reference);
    }

    return NextResponse.json({
      success: true,
      paymentUrl: chariowRes.paymentUrl,
      transactionRef: chariowRes.transId,
      reference: reference,
      amount: amount,
      currency: currency,
      pack: selectedPackKey,
    ...
    console.error("[Chariow Initiate Error]", err.message);
