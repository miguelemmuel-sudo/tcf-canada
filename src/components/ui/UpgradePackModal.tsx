  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-950 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-xl w-full p-6 sm:p-8 space-y-6 animate-in fade-in zoom-in duration-200 relative overflow-hidden">
        ...
        {/* Admin Badge Banner */}
        {adminMode && (
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-3.5 flex items-center gap-3 text-amber-600 dark:text-amber-400 text-xs sm:text-sm font-semibold">
            <Sparkles className="h-5 w-5 shrink-0 animate-pulse text-amber-500" />
            <span>
              <strong>Mode Administrateur Miguel :</strong> Vous pouvez tester le flux monétique réel Fapshi ou activer directement le pack gratuitement.
            </span>
          </div>
        )}
        ...
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            <div className="h-11 w-11 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 text-slate-950 flex items-center justify-center font-bold shadow-md shadow-amber-500/20">
              <Crown className="h-6 w-6" />
            </div>
            <div>
              <h3 className="font-black text-lg text-slate-900 dark:text-white">Souscrire / Mettre à niveau</h3>
              <p className="text-xs text-slate-500">Choisissez votre formule TCF Canada et réglez en toute sécurité.</p>
            </div>
          </div>

          <button onClick={onClose} disabled={loading} className="p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
            <X className="h-5 w-5" />
          </button>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between items-start">
                  ...
                  <span className="font-black text-base text-amber-600 dark:text-amber-400 shrink-0 ml-2">
                    {p.price}
                  </span>
                </div>
              </div>
        ...
        {/* Info box Fapshi */}
        <div className="bg-slate-50 dark:bg-slate-900/80 rounded-2xl p-4 border border-slate-200/60 dark:border-slate-800 flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400">
          <ShieldCheck className="h-6 w-6 text-emerald-600 shrink-0" />
          <p className="leading-relaxed">
            <strong className="font-bold text-slate-900 dark:text-white block">Paiement 100% sécurisé via Fapshi :</strong>
            Accepte MTN Mobile Money, Orange Money, Moov, Wave, ainsi que les cartes Visa & Mastercard. Changement transparent et conservation de tous vos acquis.
          </p>
        </div>
          ...
          {adminMode ? (
            <div className="space-y-2.5">
              <button
                ...
                disabled={loading}
                className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-black text-xs sm:text-sm shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span>Tester le paiement Fapshi (Mode Réel / Monétique)</span>
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-3 rounded-2xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-700 dark:text-amber-300 border border-amber-500/30 font-bold text-xs transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
                <Sparkles className="h-3.5 w-3.5" />
                <span>Confirmer gratuitement ({PACK_CONFIGS[selectedPack].name})</span>
              </button>
            </div>
          ) : (
            <div className="flex gap-3">
              <button
                ...
                onClick={onClose}
                disabled={loading}
                className="py-3 px-5 rounded-2xl border border-slate-200 dark:border-slate-800 font-bold text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900"
              >
                Annuler
              </button>
              ...
              <button
                ...
                onClick={handleFapshiPayment}
                disabled={loading}
                className="flex-1 py-3.5 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-black text-sm shadow-lg shadow-amber-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {loading ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin text-slate-950" />
                    <span>Redirection Fapshi en cours...</span>
                  </>
                ) : (
                  <>
                    <CreditCard className="h-4 w-4 text-slate-950" />
                    <span>S'abonner via Fapshi ({PACK_CONFIGS[selectedPack].price})</span>
